...
ADMIN_ID = 5633221424   # <--- Замени на свой реальный ID!
...
def init_db():
  ...
  cursor.execute(
      ...
      " Реклама: Наш спонсор — @example')"
  )
  cursor.execute(
      "INSERT OR IGNORE INTO settings (key, value) VALUES ('ad_status',"
      " 'OFF')"
  )
  cursor.execute(
...
async def admin_users(message: types.Message):
  ...
  cursor = conn.cursor()
  cursor.execute(
      "SELECT user_id, username, full_name, joined_date FROM users ORDER BY"
      " joined_date DESC LIMIT 15"
  )
...
async def admin_set_photo(message: types.Message):
  ...
  if len(args) < 2:
    await message.answer(
        "⚠️ Укажите прямую ссылку на картинку или отправьте ID файла."
    )
    return
  photo_val = args[1]
  conn = sqlite3.connect("bot_database.db")
...
async def handle_url(message: types.Message):
  ...
  if not any(domain in url for domain in supported_domains):
    await message.answer(
        "⚠️ Пожалуйста, отправь корректную ссылку на **YouTube**, **TikTok**"
        " или **Instagram**.",
        parse_mode="Markdown",
    )
    return
...
async def handle_other_text(message: types.Message):
  await message.answer(
      "❓ Я не понял это сообщение. Отправь мне **ссылку** на видео из YouTube,"
...
async def main():
  logging.info(
      "Бот запущен и поддерживает YouTube, TikTok, Instagram с рекламой!"
  )
